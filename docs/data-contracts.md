...
- **Wizard "⚙️ Thiết lập domain riêng tự động"** trong popup Kết nối (`#bpWizOpenBtn` → `#bpWizard`) — tự động hoá đúng quy trình `cloudflared tunnel login/create/route dns` (trước đây phải gõ tay terminal), và tự điền cả `tunnelName` lẫn `publicUrl` khi xong (không cần gõ tay `#bpPublicUrl` nữa cho Named Tunnel).
