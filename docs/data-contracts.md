...
- Cấu hình lưu ở `app.getPath('userData')/band-comm.json`, **không** đi qua `src/schema.js` / `migrateItem`. `src/band-comm/store.js` tự chuẩn hoá field thiếu khi load.
  - `room` (`name`, `code` — ID phòng 6 ký tự IN HOA, bảng chữ `23456789ABCDEFGHJKMNPQRSTUVWXYZ` (bỏ `0/O/1/I/L`), sinh 1 lần bằng `randomRoomCode()`, gõ tay được — KHÔNG phải bí mật (giống Meeting ID Zoom, xác định đúng phòng chứ không xác thực), validate ở `/api/join`/`/api/login` trước cả `password`/`username`; `password` 4–12 ký tự chữ+số (bỏ ký tự dễ nhầm 0/O/1/l/I) — sinh ngẫu nhiên nếu thiếu, vẫn đọc được `pin` số 4-8 chữ số của bản cài cũ trước khi đổi tên field; `passwordSetAt` — timestamp lần đặt/đổi mật khẩu gần nhất, `store.save()` tự stamp lại khi `password` thật sự đổi giá trị; sidebar dùng để nhắc đổi mật khẩu sau 7 ngày, `hostname` mặc định `worship`), `port` (mặc định 7071 — tự dò cổng khác nếu bị Windows/Hyper-V reserve, cổng chốt được lưu lại)
  - `publicUrl` — URL công khai (Cloudflare Tunnel / domain) để QR trỏ ra; rỗng = chỉ LAN. **Không còn dùng sau GĐ2** — trang join giờ cố định `https://channel.worship-official.link/m/` cho mọi phòng, xem mục "Đăng nhập tài khoản trung tâm" bên dưới; mục "Named Tunnel" cuối file mô tả cơ chế cũ trước GĐ2, chỉ còn giá trị tham khảo lịch sử.
  ...
  - `profiles` — backup bộ nút cá nhân theo `profileId`: `{ name, updatedAt, buttons:[{id,label,icon,group}] }`
...
- Envelope tin nhắn trên dây (WebSocket + IPC `band-comm-event`): `{ id, ts, type, from:{clientId,name}, to, refId, buttonId, dedupKey, text, meta }`. Không còn `role` (band/leader) — mọi client trên điện thoại đối xử như nhau, phân biệt duy nhất là `clientId === 'operator'`.
...
- Token phiên = `clientId.issued.<b64url(name)>.<b64url(profileId)>.<hmac>`; secret ký sinh mới mỗi lần bật server (restart server = mọi phone phải vào lại). Token cũng tự hết hạn sau `TOKEN_MAX_AGE_MS` = 12 giờ kể từ `issued` (dù server không restart) — phone bị `401`, `comm/mobile/app.js`'s `scheduleReconnect()` tự xoá token cũ + bung lại màn hình nhập mật khẩu phòng, không cần code thêm phía mobile. `profileId` mang theo trong token để mọi lần rebuild client record từ token (reconnect WS, request HTTP sau restart RAM) đều giữ nguyên — đây là danh tính dùng để xác định "chủ ảnh" cho quyền tự xoá trong gallery (xem dưới), không phải chỉ có lúc `/api/join`. Khi đăng nhập qua tài khoản (xem "Đăng nhập tài khoản" bên dưới), `profileId` = `account.id` chứ không phải id ngẫu nhiên phía client — mọi cơ chế theo-`profileId` (bộ nút cảnh báo, `ownerId` gallery) tự động hoạt động đúng theo tài khoản, không cần sửa gì thêm.
- HTTP endpoints: `GET /api/mode`, `POST /api/join`, `POST /api/login`, `POST /api/join-room`, `POST /api/change-password`, `POST /api/message`, `POST /api/ping`, `POST|GET /api/profile`, `POST /api/leave`. Downstream là **WebSocket** `GET /api/ws?token=…&since=<lastEnvelopeId>` (không còn `/api/stream` SSE — Cloudflare Tunnel buffer streaming HTTP). Mọi route trừ `mode`/`join`/`login`/`join-room` cần token.
- `POST /api/join` và `POST /api/login` đều nhận thêm `code` (= `room.code`) — sai code trả `403 { error: 'Sai ID phòng' }` **trước khi** xét tới `password`/`username`/`idToken`, không gộp chung lỗi để người dùng biết đúng bước nào sai.
- **Chống brute force mật khẩu phòng** trên `/api/join` (`joinAttempts` trong `server.js`, theo IP nguồn `req.socket.remoteAddress`, chỉ ở RAM — mất khi restart server): 5 lần sai đầu miễn phí (khớp gõ nhầm tay), từ lần sai thứ 5 trở đi khoá tăng dần — 30s (fails 5-9) → 5 phút (10-19) → 30 phút (≥20); trả `429` kèm header `Retry-After` khi đang bị khoá, kể cả nếu mật khẩu gõ đúng lúc đó (không có cách nào bỏ qua khoá bằng cách đoán đúng). Mật khẩu phòng sinh mặc định 6 ký tự chữ+số (~57^6 khả năng — đủ vô hiệu hoá brute-force tự nó, không cần thêm allowlist theo từng phòng; trước đây PIN chỉ 4 số/10.000 khả năng nên khoá backoff này gánh gần hết việc chặn). Sinh qua Cloudflare Tunnel: mọi request đều tới từ `127.0.0.1` (cloudflared proxy nội bộ) nên khoá theo IP lúc đó thành khoá dùng chung cho toàn bộ traffic ngoài LAN — chấp nhận được, ưu tiên hơn không có gì chặn. Dọn entry cũ (> 1 giờ không hoạt động) mỗi nhịp heartbeat (`pruneJoinAttempts`, cùng interval `HEARTBEAT_MS` dọn presence). Operator có thể bấm 🔄 cạnh Mật khẩu trong sidebar để đổi ngay nếu nghi bị dò.
...
- Khi bật: **operator (laptop) là nơi DUY NHẤT tạo/sửa/xoá tài khoản** — không có endpoint tự đăng ký. Dữ liệu tách file riêng `userData/band-comm-accounts.json` (`src/band-comm/accounts.js`, mirror pattern `store.js`) — lý do tách giống gallery: giảm đụng độ khi sửa đồng thời.
  - Mỗi account: `{ id, username, name, passwordHash, passwordSalt, active, createdAt, lastLoginAt, mustChangePassword }`. `passwordHash`/`Salt` = `crypto.scryptSync(password, salt, 64)` (built-in Node, 0 dependency). **Giới hạn thật của hash này:** chỉ chống lộ file trần, KHÔNG chống brute-force offline nếu file lộ VÀ password ngắn/yếu — lớp phòng thủ thật là rate-limit theo `accountId` ở `/api/login` (xem dưới), không phải việc có hash hay không.
  - Mật khẩu **do operator tự đặt lúc tạo** (không có "hệ thống tự sinh rồi hiện 1 lần"), và **band member không tự đổi được ngoại trừ 1 trường hợp hẹp** (xem `mustChangePassword` dưới) — mọi thao tác reset khác chỉ operator làm được, qua tab "Tài khoản" (Settings → Media & Band).
  - `mustChangePassword` — operator tick lúc tạo account (mặc định tắt) hoặc **bật mặc định** mỗi lần operator tự reset mật khẩu account đã có (`accounts.updatePassword()`, tham số `mustChangePassword` mặc định `true`). `/api/login`/`/api/join-room` trả cờ này trong response `finishLogin()`; mobile client (`showMustChangePassword()` trong `app.js`) chặn vào phòng, bắt gọi **`POST /api/change-password { currentPassword, newPassword }`** (cần token hợp lệ + đúng mật khẩu hiện tại, qua `accounts.changeOwnPassword()`) trước khi tiếp tục — đây là NGOẠI LỆ HẸP duy nhất cho "band member không tự đổi mật khẩu": chỉ dùng được khi đang thực hiện đúng yêu cầu operator đặt ra, không phải nút "đổi mật khẩu" tự do. Đổi thành công tự xoá cờ.
- `GET /api/mode` (public, không cần token, không lộ roster) → `{ accountsEnabled, passwordRequiredWithAccounts, authMode }` — mobile dùng để biết vẽ màn hình nào lúc tải trang (`authMode` xem mục "Đăng nhập tài khoản trung tâm" bên dưới).
- `POST /api/login` `{ username, password }` — verify qua `accounts.js`, lấy `name` thật từ account (client không tự khai). **Không có** `GET/POST /api/accounts` liệt kê roster — cố tình, tránh lộ danh sách tên qua Cloudflare Tunnel công khai (mặc định bật, xem CLAUDE.md). Trả:
  - `room.passwordRequiredWithAccounts=false` (mặc định) → token đầy đủ ngay (cùng shape response với `/api/join`, cộng thêm `name`).
  - `true` → `{ needsRoomPassword: true, tempToken }`. `tempToken` = `crypto.randomBytes(16).toString('hex')`, lưu trong `Map pendingLogins` (accountId/name/expiresAt, TTL 5 phút, dọn cùng nhịp heartbeat) — **không đi qua `makeToken()`/`verifyToken()`** (không đủ số phần cách nhau bằng dấu chấm) nên mọi route khác tự động `401` nếu lỡ dùng nhầm, fail-closed theo đúng cấu trúc dữ liệu chứ không phải vì có route nào tự nhớ kiểm tra 1 cờ.
- `POST /api/join-room` `{ tempToken, password }` (chỉ gọi khi bước trên báo `needsRoomPassword`) — verify `password` đúng `room.password` hiện có (dùng lại nguyên cơ chế mật khẩu phòng, không phải field mới) → đổi `tempToken` lấy token đầy đủ.
- Rate-limit (dùng lại `joinAttempts`/`passwordBackoffMs` của mật khẩu phòng, xem mục "Chống brute force mật khẩu phòng" phía trên): `/api/login` khoá theo **cả** `login-ip:<ip>` **và** `login-acct:<username>` (gõ sai password của chính mình nhiều lần không khoá lây người khác cùng IP/Tunnel, nhưng account đó vẫn bị khoá dù đổi IP); `/api/join-room` khoá theo `joinroom-ip:<ip>`.
- `/api/join` (luồng mật khẩu phòng cũ) chặn thêm 1 điều kiện khi cả 2 luồng cùng bật (`accountsEnabled=true` + vẫn cho mật khẩu phòng chạy song song): `body.profileId` client tự khai **không được trùng bất kỳ `accounts[].id` thật nào** — nếu trùng, coi như không hợp lệ (server tự sinh id khác) — chặn 1 client biết/đoán đúng id 1 account thật chiếm quyền xoá ảnh/bộ nút của account đó mà không cần đăng nhập.
- Đổi mật khẩu / khoá / xoá 1 account, hoặc bật/tắt `accountsEnabled`/`room.passwordRequiredWithAccounts` → gọi `commServer.rotateSecret()` (sinh lại secret ký HMAC) — mọi token đang tồn tại verify-fail ngay ở lần gọi kế tiếp, client tự bung màn đăng nhập lại qua đúng luồng `401` đã có, không cần cơ chế "kick" riêng theo từng WebSocket.

### Đăng nhập tài khoản trung tâm (`authMode: 'cognito'`) — cloud/identity-plan.md

- Song song với mô hình cục bộ ở trên, **không thay thế** — `room.authMode` (`'local'` mặc định | `'cognito'`) chọn NƠI `/api/login` xác thực; bước mật khẩu phòng (`/api/join-room`) và mọi luồng LAN thật (WS, gallery, setlist) **giữ nguyên 100%** dù chọn mô hình nào. Đổi trong Settings → Media & Band → "Kiểu tài khoản" (đổi cũng `rotateSecret()` như đổi `accountsEnabled`).
- Danh tính đến từ **Cloudflare Worker `band-identity`** (`cloud/identity/src/worker.js`, domain `identity.worship-official.link`) gọi AWS Cognito User Pool `worship-band-users` (region `ap-southeast-2`) qua SigV4 (`aws4fetch`) — Worker là NƠI DUY NHẤT giữ AWS credentials (Worker secrets `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`, không commit vào repo). 3 endpoint public, không qua LAN server:
  - `POST /request-access { email }` → `AdminCreateUser` (mật khẩu tạm tự sinh, `MessageAction: SUPPRESS`) + gửi mail qua Resend. Luôn trả `{ ok: true }` dù email đã có tài khoản hay chưa — tránh lộ danh sách email đã đăng ký.
  - `POST /forgot-password { email }` → `AdminSetUserPassword(Permanent:false)` (đặt lại thành mật khẩu TẠM mới) + gửi mail qua Resend, tái dùng `sendAccessEmail()` của `/request-access`. Cognito tự bắt `NEW_PASSWORD_REQUIRED` ở lần `/login` kế tiếp — dùng chung luồng đổi-mật-khẩu-tạm đã có, KHÔNG dùng Cognito's `ForgotPassword` built-in (cần cấu hình SES riêng). Cùng nguyên tắc không lộ tồn tại email như `/request-access` — luôn trả `{ ok: true }`, lỗi `UserNotFoundException` không log/không phản ánh ra response.
  - `POST /login { email, password }` → `AdminInitiateAuth`. Trả `{ challenge: 'NEW_PASSWORD_REQUIRED', session }` nếu còn dùng mật khẩu tạm, hoặc gọi lại `POST /login { email, session, newPassword }` (→ `AdminRespondToAuthChallenge`) để hoàn tất. Thành công → `{ idToken, accessToken, refreshToken, expiresIn }`.
  - `POST /refresh { email, refreshToken }` → JWT mới không cần nhập lại mật khẩu.
  - Rate-limit theo IP/email qua KV `IDENTITY_RL` (cùng kiểu đếm-theo-cửa-sổ như `cloud/worker/src/worker.js`).
- `comm/mobile/`: `GET /api/mode` trả thêm `authMode`; khi `'cognito'`, màn join hiện `#joinCognitoFields` (tên hiển thị tự khai + email/mật khẩu) gọi thẳng Worker ở trên, KHÔNG qua LAN server; xong mới POST `idToken` cho LAN server. "Yêu cầu qua email" gọi `/request-access` trực tiếp từ phone.
- `server.js`: `POST /api/login` khi `authMode='cognito'` nhận `{ idToken, name }` (name tự khai như `/api/join`, KHÔNG xác thực qua Cognito) — verify **offline** bằng `src/band-comm/cognito-jwks.js` (`jose` + JWKS cache tại `userData/cognito-jwks.json`, tự refresh 24h/lần khi có mạng, dùng cache cũ khi offline; verify chữ ký + `iss`/`aud`/`token_use='id'`, KHÔNG phát sinh request mạng nếu key đã có sẵn trong cache — đúng nguyên tắc LAN-first, phòng họp mất mạng ngoài LAN vẫn verify được). `profileId = sub` claim (id Cognito vĩnh viễn cho 1 người, không đổi dù đổi tên/mật khẩu) — tái dùng nguyên cơ chế `profileId` đã có (gallery `ownerId`, bộ nút cảnh báo), không cần sửa gì thêm. Sai chữ ký/hết hạn → `401` chung chung (không phân biệt lý do).
- `POST /api/join-room` cho nhánh `cognito`: **không** re-fetch qua `accountsStore` (không tồn tại cho danh tính trung tâm) — dùng thẳng `name` đã verify ở bước 1, lưu sẵn trong `pendingLogins`.

### Gate đăng nhập operator (bắt buộc mọi bản cài)

- Khác hẳn tài khoản tài khoản trung tâm ở trên (đó là band member đăng nhập từ điện thoại) — đây là **chính operator** đăng nhập trên máy chiếu bằng cùng tài khoản Cognito trung tâm, TRƯỚC KHI Kênh Band được phép khởi động (`main.js`'s `app.whenReady()` chỉ gọi `startBandComm()` khi `bandOperatorAuthStore.isLoggedIn()`). **Bắt buộc cho MỌI bản cài** (không còn diện miễn trừ theo kiểu "chỉ bản cài mới" — đã bỏ hẳn field `requireOperatorLogin`).
- Phiên lưu ở `userData/band-comm-operator.json` (`src/band-comm/operator-auth.js`) — chỉ lưu token (`idToken`/`accessToken`/`refreshToken`/`expiresAt`), không có mật khẩu nào đi qua đây. Refresh token Cognito sống ~30 ngày.
- IPC (`preload.js`'s `electronAPI.bandComm.operatorAuth`): `getStatus`, `requestAccess`, `forgotPassword`, `login`, `setNewPassword`, `logout` — gọi thẳng Worker `band-identity` (`/request-access`, `/forgot-password`, `/login`) qua `main.js`, không qua LAN server.
- UI: `#bandGateModal` trong `index.html`, tự hiện khi `window.__bandGateCheck(onPass)` (gọi trước lúc mở sidebar `#bandPanel`) thấy `!loggedIn`. Đăng xuất (`band-operator-auth-logout`) dừng hẳn `commServer` ngay để trạng thái "đã đăng xuất" là thật.
...
- Hạ tầng: Cloudflare Worker (`cloud/worker/src/worker.js`) + Workers KV, domain riêng `channel.worship-official.link` (đổi tên từ `api.` — đây là điểm vào dùng chung cho MỌI user, đã phân chia cách ly theo phòng qua Durable Object nên tên `channel` phản ánh đúng bản chất hơn `api`; Route 53 chỉ dùng để trỏ NS sang Cloudflare — DNS/route thật nằm ở Cloudflare). Deploy bằng `npx wrangler deploy` trong `cloud/worker/`. Route khai báo kiểu **Custom Domain** (`custom_domain = true` trong `wrangler.toml`) chứ không phải `routes` + `zone_name` thường — cách này để `wrangler deploy` tự tạo DNS record cần thiết, không phải chỉnh DNS zone thủ công.
- `cfg.cloudRoomId`: UUID sinh 1 lần trong `band-comm.json` (`src/band-comm/store.js`), namespace hoá dữ liệu trên KV (`sl:<roomId>:<id>`, `ack:<roomId>:<id>`, TTL 7 ngày). Không phải bí mật mạnh — đủ khó đoán cho quy mô LAN nội bộ, mật khẩu phòng thật vẫn ở tầng server local. Trả về cho điện thoại trong response `POST /api/join`.
- Worker endpoints (không cần token — xác thực bằng `roomId` khó đoán): `POST /setlist {roomId, setlist:{id,name,from:{name},items}}`, `GET /setlist?roomId=` → `{setlists}`, `POST /setlist/ack {roomId,id}`, `GET /setlist/ack?roomId=&ids=a,b,c` → `{acked}`, `GET /health`.
...
### Named Tunnel (thay Quick Tunnel) — domain cố định cho band-comm [ĐÃ NGỪNG DÙNG SAU GĐ2]

> **GĐ2 đã bỏ hẳn cơ chế này** — `main.js`'s `syncBandTunnel()` vẫn còn trong file nhưng không còn nơi nào gọi tới (band-comm giờ luôn nối ra ngoài qua `relay-client.js` → Cloudflare Durable Object, 100% online, không LAN server nào để tunnel tới). Toàn bộ mục này mô tả kiến trúc CŨ, giữ lại chỉ để tham khảo lịch sử/debug nếu cần đọc lại code cũ trong `src/band-comm/server.js`. Tunnel `blessing-band`/`blessing-church`/`blessing-church-local` và `cloud/tunnel/start-tunnel.bat` đã bị xoá.

- Setup 1 lần trên máy vận hành (đã làm, LỊCH SỬ): `cloudflared tunnel login` (đăng nhập trình duyệt, tải `cert.pem`) → `cloudflared tunnel create blessing-band` (sinh tunnel id + file credentials JSON, cả 2 nằm trong `%USERPROFILE%\.cloudflared\`, KHÔNG commit vào repo) → `cloudflared tunnel route dns blessing-band blessing.worship-official.link` (tạo CNAME trỏ tunnel).
