...
## 13. GĐ1 — chuẩn bị chuyển online: Room Code, gate đăng nhập bắt buộc mọi bản cài, mustChangePassword, Quên mật khẩu (2026-09-18)

Xuất phát từ quyết định kiến trúc lớn của user: bỏ hẳn LAN cho Kênh Band, chuyển 100% online qua relay trung tâm (Durable Objects, chưa triển khai — sẽ ở §14+ khi tới). App trình chiếu (presentation) vẫn offline, chỉ riêng Kênh Band đổi. GĐ1 là phần **additive**, chạy tốt trên nền LAN hiện tại, không phụ thuộc việc relay tương lai có xong hay không:

- **Room Code** (`room.code`, 6 ký tự `23456789ABCDEFGHJKMNPQRSTUVWXYZ`, IN HOA, gõ tay được) — `store.js`'s `randomRoomCode()`. Sinh cho mọi bản cài (kể cả cũ, `load()` tự phát hiện `!raw.room.code` và ghi lại ngay, không đợi `save()` lần sau). Không phải bí mật (giống Meeting ID Zoom) — xác định ĐÚNG PHÒNG, mật khẩu phòng mới là lớp xác thực thật. Validate ở `/api/join` và `/api/login` (server.js) trước khi xét tới password/username — sai code trả riêng lỗi "Sai ID phòng", không gộp vào "sai mật khẩu". Hiện trong sidebar (`#bpRoomCode`, cạnh `#bpPassword`) và cả 3 form join mobile (`#roomCode`, `#roomCode2`, `#roomCode3`).
- **`requireOperatorLogin` — bỏ hẳn field này, gate đăng nhập operator giờ bắt buộc MỌI bản cài** (trước đây chỉ bản cài mới, có grandfather clause cho máy cũ — đã bỏ theo yêu cầu trực tiếp của user, họ không thấy form login trên máy cũ của mình). `main.js`'s `app.whenReady()` chỉ còn check `bandOperatorAuthStore.isLoggedIn()`.
- **`mustChangePassword`** — field mới trên account local (`accounts.js`). Operator tick "Bắt đổi mật khẩu sau lần đăng nhập đầu" lúc tạo tài khoản (mặc định `false`) hoặc **mặc định `true`** khi operator tự reset mật khẩu 1 account đã có (`updatePassword()`). `/api/login`/`/api/join-room` trả cờ này trong response; mobile chặn vào phòng, bắt đổi mật khẩu qua **`POST /api/change-password`** (endpoint mới, cần đúng mật khẩu hiện tại) trước khi vào màn chính — **đây là ngoại lệ hẹp với D12** ("band member không tự đổi mật khẩu được"): band member CHỈ tự đổi được khi đang thực hiện đúng yêu cầu `mustChangePassword` do operator bật, không phải tính năng "tự đổi mật khẩu bất cứ lúc nào".
- **Quên mật khẩu** (tài khoản trung tâm Cognito, không liên quan accounts.js local) — `cloud/identity/src/worker.js`'s **`POST /forgot-password`**, tái dùng `AdminSetUserPassword(Permanent:false)` + `sendAccessEmail()` có sẵn từ `/request-access`. Giữ nguyên nguyên tắc không lộ email đã có tài khoản hay chưa — response luôn `{ok:true}`. Có ở cả màn đăng nhập Cognito mobile (`#ciForgotPasswordBtn`) và gate đăng nhập operator (`#opgateForgotBtn`).

**Sửa:** `store.js` (Room Code, bỏ `requireOperatorLogin`), `accounts.js` (`mustChangePassword`, `changeOwnPassword()`), `server.js` (validate code, `/api/change-password`, `getStatus()` trả `code`), `main.js`/`preload.js` (bỏ gate cũ, IPC `band-operator-forgot-password`), `cloud/identity/src/worker.js` (`/forgot-password`), `comm/mobile/index.html`+`app.js` (field ID phòng x3, màn bắt đổi mật khẩu, nút Quên mật khẩu), `index.html` operator (hiện Room Code sidebar, checkbox mustChangePassword, badge "Chờ đổi mật khẩu", nút Quên mật khẩu ở gate).

**Chưa làm (GĐ2-4, chờ xác nhận kiến trúc relay từ cả 2 phía user):** Durable Object relay thay LAN server, nối lại `comm/mobile/app.js` + sidebar vào relay mới, dọn code chết (LAN HTTP server, `mdns.js`, `syncBandTunnel()`/wizard tunnel, `ws.js`).

## 14. GĐ2 — Durable Object relay (2026-09-18, ĐANG LÀM DỞ)

User xác nhận dứt khoát (2 lần, gay gắt lần 2) chuyển Kênh Band sang 100% online — bỏ hẳn LAN HTTP+WS server, mDNS, Cloudflare Tunnel. Kiến trúc mới: **cả operator (laptop) LẪN band member (điện thoại) đều là WebSocket CLIENT nối ra ngoài** tới 1 relay trung tâm — không còn ai host server, nên không cần LAN discovery, không cần tunnel, không có cổng nào mở ra Internet từ máy operator (đúng ý "giảm attack surface" ở yêu cầu gốc).

Công nghệ: **Cloudflare Durable Object**, 1 instance/phòng (định danh bằng `cloudRoomId` đã có sẵn), thêm vào **cùng Worker `band-comm-relay`** đang chạy (`cloud/worker/`, domain `channel.worship-official.link` — đổi tên từ `api.` vì đây là điểm vào dùng chung cho toàn bộ user, đã phân chia theo phòng rồi) — không tạo Worker/domain riêng, tận dụng tên `band-comm-relay` vốn đã đặt sẵn cho việc này từ đầu.

### Đã làm + test thật (KHÔNG đoán — 2 vòng test qua `wrangler dev` + WebSocket thật, 37/37 case PASS tổng cộng)

- **`cloud/worker/src/room-relay.js`** (mới) — class `RoomRelay` (Durable Object):
  - `POST /admin/config` — operator bootstrap/cập nhật `{name, code, password}`, xác thực bằng `X-Admin-Secret` (secret riêng của máy operator, KHÁC `room.password` — xem `store.js`'s `relayAdminSecret` bên dưới). Lần gọi ĐẦU TIÊN tự ghim secret (bootstrap), sau đó bắt buộc khớp. Đổi `password` tự `rotateSecret()` (mọi token cũ verify-fail ngay) — y hệt hành vi `commServer.rotateSecret()` cũ.
  - `POST /join` — y hệt UX `/api/join` LAN cũ: `{name, code, password}`, validate code TRƯỚC password, cấp token 5 phần (HMAC, Web Crypto thay `crypto.scryptSync`/`createHmac` của Node — Workers runtime không có 2 hàm đó).
  - `GET /ws?token=…` (band member, sau `/join`) hoặc `GET /ws?adminSecret=…` (operator, KHÔNG cần qua `/join` — danh tính cố định `clientId:'operator', name:'Người chiếu máy'`, y hệt hằng số `OPERATOR` ở server.js cũ) — dùng **WebSocket Hibernation API** (`ctx.acceptWebSocket`), state sống sót qua evict nhờ `ws.serializeAttachment()` (per-connection) + `ctx.storage` (durable: config, secret, ring buffer 120 tin).
  - Message qua WS: `kind:'message'` (band gửi alert, ai join hợp lệ cũng gửi được) · `kind:'text'/'ack'/'resolve'` (**chỉ operator** — kiểm tra `attachment.isOperator`, band member gửi bị lặng lẽ bỏ qua, không ai nhận) · `kind:'ping'` → `pong`.
  - Reconnect (`?since=<lastEnvelopeId>`) replay đúng phần ring buffer bị bỏ lỡ — đã test riêng.
  - **Chống brute-force `/join` + `/login`** — port curve `passwordBackoffMs` y hệt `server.js` cũ (RAM per-DO-instance, không phải `ctx.storage` — chấp nhận reset khi evict, cùng đánh đổi LAN cũ chấp nhận reset khi restart). Khoá theo `CF-Connecting-IP` (header Cloudflare edge gắn sẵn, sống sót qua lượt forward worker.js→DO).
  - **Tài khoản local** (`cloud/worker/src/accounts-webcrypto.js`, mới) — đối chiếu `src/band-comm/accounts.js`, CHỈ khác thuật toán hash: PBKDF2-HMAC-SHA256 210.000 vòng qua Web Crypto thay `crypto.scryptSync` (Workers không có hàm đó). Endpoint operator-only (`X-Admin-Secret`): `POST /admin/accounts/{create,update,update-password,set-active,remove}`, `GET /admin/accounts/list`. Endpoint band member: `POST /login` (username/password), `POST /join-room` (bước 2 nếu `passwordRequiredWithAccounts`), `POST /change-password` (cần token — chỉ dùng được khi `mustChangePassword=true`, y hệt ngoại lệ hẹp đã chốt hôm qua).
  - **Tài khoản trung tâm Cognito** (`cloud/worker/src/cognito-verify.js`, mới) — verify RS256 JWT bằng Web Crypto thuần (không thêm dependency `jose`), cache JWKS trong RAM per-DO-instance (TTL 24h) — khác Node's `cognito-jwks.js` (cache ra đĩa + fallback offline) vì relay LUÔN có mạng, không cần đường offline. `POST /login {idToken, name}` khi `authMode='cognito'`.
- **`cloud/worker/src/worker.js`** — thêm route `/api/room/<cloudRoomId>/…` forward request nguyên vẹn (kể cả WS upgrade) sang đúng DO qua `env.ROOMS.idFromName(roomId)`. Các endpoint cũ (`/setlist`, `/library`, `/gallery`, `/composer`) **giữ nguyên không đổi**.
- **`cloud/worker/wrangler.toml`** — thêm binding `ROOMS` (Durable Object, SQLite-backed qua `new_sqlite_classes` — bắt buộc dạng này cho `compatibility_date` mới). Lưu ý: mọi `key = value` cấp cao nhất PHẢI đặt trước bảng `[[…]]` đầu tiên trong file, không TOML sẽ gán lộn vào bảng vừa mở (đã tái hiện + fix qua `wrangler deploy --dry-run`).
- **`src/band-comm/store.js`** — thêm `relayAdminSecret` (32 byte hex, sinh 1 lần, persist ngay cho config cũ thiếu field — cùng cơ chế `room.code` hôm qua).
- **`src/band-comm/relay-client.js`** (mới, CHƯA wire vào `main.js`) — module đối xứng `createCommServer()` cũ: `start/stop/getStatus/isRunning/operatorSend/operatorAck/operatorResolve/rotateSecret`, kết nối WS tới relay bằng `relayAdminSecret`, reconnect exponential backoff (1s→30s, cùng kiểu `comm/mobile/app.js`'s `scheduleReconnect()`), `getStatus()` bỏ hẳn field không còn ý nghĩa (`host/ip/port/publicUrl/tunnelName`) — thay bằng `mode:'relay'`. Test độc lập bằng node script thật (không qua Electron): bootstrap config, operator connect, band member connect + join THẬT qua `/api/room/.../join`, gửi/nhận 2 chiều, ack/resolve, presence, `start()` reject đúng khi relay không tới được (main.js cần throw này để báo lỗi sidebar).

**Tổng test thật đã chạy: 100+ case PASS** (qua `wrangler dev` + WebSocket thật + HTTP thật + **app Electron thật đang chạy**, KHÔNG đoán) — 23 case luồng cơ bản, 8 case operator protocol, 33 case accounts/rate-limit/gallery/profile/setlist (kể cả bắt được và fix 1 lỗi bảo mật thật: band member không truyền `profileId` xoá được ảnh người khác — đã vá), 5 case routing Cognito qua token rác, 9 case crypto RS256, 30 case relay-client.js (thêm accounts admin + gallery + syncLibraryToCloud + onSetlist), 9 case app.js chạy trong VM thật (join → WS thật → toast hiện đúng). **Giới hạn còn lại:** chưa test luồng Cognito THẬT với AWS (cần tài khoản test thật).

**`comm/mobile/` đã host trên Worker** (`/m/`, mount qua Cloudflare Assets binding, `html_handling:"none"` để tự kiểm soát URL, có redirect `/m` → `/m/` để `<script src="app.js">` tương đối resolve đúng).

**`comm/mobile/app.js` đã rewire xong** — toàn bộ networking đổi từ LAN sang relay: `roomUrl()`/`roomWsUrl()` dùng `state.roomCode` (không phải cloudRoomId), tự dò `/mode` theo code (từ `?room=` hoặc gõ tay + blur), nút bấm/compose gửi qua WS (`kind:'message'`) thay vì `fetch('api/message')`, gallery/profile/setlist đổi sang `roomUrl('/gallery/...')` với token query-string, `fetchLibrary()` trỏ thẳng Worker-level `/library` (không qua DO). Test qua VM thật (Node `vm` module + DOM giả tối thiểu, load ĐÚNG file app.js, không copy lại logic) — join thật, WS thật, gửi/nhận 2 chiều, toast hiện đúng.

**`main.js` đã rewire xong** — `createCommServer` → `createRelayClient`, bỏ hẳn `bandMdns`/`syncBandTunnel()` khỏi luồng chạy chính (`startBandComm()`, `band-comm-save-config`), `band-accounts-*` IPC đổi sang `commServer.accountsX()` (tài khoản giờ sống trong DO, không còn `band-comm-accounts.json`), gallery IPC không cần đổi gì (interface đã khớp sẵn). **Test bằng app Electron THẬT** (không phải node script): seed 1 session operator hợp lệ (bypass yêu cầu đăng nhập Cognito thật — chỉ cần biết field `refreshToken` không rỗng là đủ theo `operator-auth.js`'s `isLoggedIn()`), `BAND_RELAY_BASE` trỏ `wrangler dev` local, mở app thật → xác nhận qua `curl`: relay nhận đúng `admin/config` (`configured:true`), operator WebSocket THẬT xuất hiện trong `/presence`, gửi tin nhắn thật từ 1 band member giả tới app đang chạy thành công. Đây là xác nhận mạnh nhất có thể làm được mà không cần bạn tự tay đăng nhập.

**Sidebar `index.html` đã dọn cho model relay** — QR/URL giờ chỉ 1 link cố định (`channel.worship-official.link/m/?room=<code>`, đổi tên từ `api.` ngày 2026-09-19) thay vì IP LAN/host.local/Public URL đa chế độ cũ; xoá nút "Mở cổng Firewall" (không còn cổng LAN nào để mở) — **phát hiện & vá 2 lỗi crash thật** trong lúc dọn (2 chỗ gọi `addEventListener` vào phần tử HTML đã xoá, `$('bpFw')`/`$('bpPublicUrl')`, đáng lẽ sẽ làm vỡ hẳn sidebar init nếu không rà kỹ). Popup gate đăng nhập operator cũng đã thu nhỏ theo yêu cầu (420px→320px, giảm padding/font đồng bộ).

### Còn lại (chưa làm — liệt kê đủ để không ai tưởng nhầm đã xong)

1. **Gallery: quyết định nơi lưu ảnh gốc** — đã lưu thẳng R2 qua DO (`addImage()`/`removeImage()` trong `room-relay.js`, test kỹ), không còn cache đĩa cục bộ nào — coi như đã xong, không phải "chưa quyết" nữa.
2. Test **điện thoại thật** (không chỉ VM giả lập) sau khi deploy production — chưa làm, cần domain thật hoạt động trước.
3. Dọn code chết: `src/band-comm/mdns.js`, `src/band-comm/ws.js`, `src/band-comm/server.js`, `src/band-comm/accounts.js`, `src/band-comm/cognito-jwks.js` (không còn require() ở đâu trong `main.js` nữa, nhưng CHƯA xoá file — vẫn nằm im trong repo), tunnel wizard IPC (`band-comm-tunnel-*` trong `main.js`) + UI wizard trong `index.html` Settings (không còn tác dụng thật nhưng chưa gỡ hẳn), `band-comm-open-firewall` IPC (không còn nút gọi tới).
4. Deploy `cloud/worker/` lên production (`wrangler deploy` — mọi test ở trên đều qua `wrangler dev` local, CHƯA chạm production).
