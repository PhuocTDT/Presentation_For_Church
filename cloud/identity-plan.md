...
> Trạng thái: **ĐÃ TRIỂN KHAI XONG bước 1–5, còn thiếu domain email Resend** (2026-09-17). Thay thế mô hình "operator tạo tài khoản cục bộ" ở `band-comm-plan.md` §11 bằng 1 danh tính dùng chung cho mọi phòng/nhà thờ — nhưng **song song, không ép buộc** (§5). **Lớp mã phòng (`room.pin`/`pinRequiredWithAccounts`) không đổi gì cả** — chỉ đổi lớp "đăng nhập cá nhân là ai".
>
> Đã xong + verify thật (xem §6): User Pool + IAM (bước 1), Worker `band-identity` deploy tại `identity.worship-official.link` + 3 endpoint test bằng Cognito thật (bước 3), `server.js` verify JWT offline bằng `jose`+JWKS cache — test cả đường online lẫn "ngắt fetch giữa chừng vẫn verify được" (bước 4), UI `comm/mobile/` + toggle Settings (bước 5). **Còn thiếu:** domain Resend chưa verify (bước 2) — `RESEND_API_KEY` chưa set trên Worker nên `/request-access` tạo tài khoản Cognito thành công nhưng KHÔNG gửi được mail mật khẩu tạm (Worker tự log lỗi, không throw) cho tới khi việc này xong.
>
...
2. ⬜ **Domain email cho Resend** — CHƯA LÀM. Cần: tài khoản Resend (chưa xác nhận đã có hay chưa), verify domain (đề xuất subdomain `mail.worship-official.link`, DKIM/SPF/DMARC qua Cloudflare DNS đã quản sẵn), rồi `wrangler secret put RESEND_API_KEY` từ `cloud/identity/`. Tới lúc đó `/request-access` vẫn tạo được tài khoản Cognito nhưng không gửi được mật khẩu tạm qua mail.
...
4. ✅ **`server.js`: verify JWT offline** — `POST /api/login` nhận `{ idToken, name, role }` khi `authMode='cognito'`, verify bằng `src/band-comm/cognito-jwks.js` (`jose`). `POST /api/join-room` giữ nguyên (đã fix 1 bug lúc code: nhánh cognito không được gọi `accountsStore.findById()` vì id đó là Cognito `sub`, không tồn tại trong `accounts.js`). Verify thật bằng JWT thật lấy từ Worker: 16 test case PASS bao gồm **tắt hẳn `global.fetch` giữa chừng vẫn login được** (chứng minh không phát sinh request mạng khi key đã cache) + hồi quy 5 test case mô hình local vẫn PASS.
5. ✅ **Màn đăng nhập trong `comm/mobile/`** — thêm `#joinCognitoFields` (tên+vai trò tự khai + email/mật khẩu, gọi thẳng Worker) + `#joinCognitoNewPasswordFields` (đổi mật khẩu tạm) + nút "Yêu cầu qua email" gọi `/request-access`. Settings → Media & Band thêm chọn "Kiểu tài khoản" (`local`/`cognito`) — verify thật qua CDP: toggle đúng UI + `saveConfig` round-trip đúng qua IPC.
6. (Sau, nếu cần) Web Pages riêng nếu quyết định tách hẳn khỏi LAN server phục vụ tĩnh.

**Đã biết nhưng chưa làm (ngoài domain Resend ở bước 2):** chưa có refresh-token tự động ở `comm/mobile/` (JWT Cognito hết hạn ~1h chỉ dùng để bootstrap token LAN 12h lúc đăng nhập — hết 12h phải đăng nhập lại, cần mạng lúc đó) — chấp nhận được cho v1, đúng tinh thần "chỉ bắt buộc có mạng lúc đăng nhập, không bắt buộc lúc đang họp".
...
- **D19 — Domain Resend:** → "ok" (không phân biệt rõ 2 lựa chọn) — **giả định đã dùng: `mail.worship-official.link`** (`MAIL_FROM` trong `cloud/identity/wrangler.toml`), subdomain riêng tách uy tín gửi mail khỏi domain chính. **Chưa verify domain này trên Resend** (bước 2, §6) — cần xác nhận lại nếu muốn domain khác.
